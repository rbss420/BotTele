...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di Group *${mdata.subject}*,Jangan lupa save nomor owner bot ketik !creator`
				...
				teks = `Sayonara👋🏻 @${num.split('@')[0]} Semoga tenang di alam sana`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
            ...
            client.sendFile(from, sesPic, 'session.png', 'Apa', id)
            break	
				case 'gtts':	
				case 'tts':
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻 @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝗱𝗮𝗵 𝗷𝗮𝗱𝗶 𝘀𝗮𝘆𝗮𝗻𝗴'})
						fs.unlinkSync(ran)
					})
					break
